...
USERNAME=$(dialog --title "Username" --inputbox "Enter username: \n (without space, special characters, not starting with a number):" $HEIGHT $WIDTH 3>&1 1>&2 2>&3)
exitstatus=$?
if [ $exitstatus != 0 ]; then
    clear
    exit 0
...
while true; do
    PASSWORD=$(dialog --title "Password" --insecure --passwordbox "Enter password: \n (remember it, as this will be used for root,\n encrypted partition,GPG key etc):" $HEIGHT $WIDTH 3>&1 1>&2 2>&3)
    exitstatus=$?
    if [ $exitstatus != 0 ]; then
        clear
        exit 0
...
CRYPT_DEV="/dev/mapper/encrypted_vault"
...
echo "10" ; echo "# Creating partitions..."
...
chmod +x "/mnt/home/$USERNAME/Desktop/Apps/google-chrome.Appimage"  #need to check if it works
